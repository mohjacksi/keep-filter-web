                        ...
                        <label>نوع السياره</label>
                            <select name="car_category_id[]" title="اختر السيارات المناسبه"  required class="selectpicker  form-control-solid form-control" multiple data-live-search="true">
                                @foreach ($car_category as $category)
                                    <option value="{{$category->id}}" {{$product->car == $category->id ? 'selected' : ''}} >{{$category->name}}</option>
                                @endforeach
                            </select>
   ...
   <!--begin::Form-->
        ...
        <div class="card-body">
            ...
            @foreach ($product->Images as $image)
                <form class="form"  enctype="multipart/form-data" method="POST" action="{{route('product.updateImage')}}" style="margin: 5px; border: 1px solid rgb(68, 67, 67);" >
                    @csrf
                    @method('PUT')
